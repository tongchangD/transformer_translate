...
def train_model(model, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        ((time.time() - start)//60, epoch + 1, "".join('#'*(100//5)), "".join(' '*(20-(100//5))), 100, loss, epoch + 1, loss))
...
def main():
    ...
    parser.add_argument('-src_data',default="./data/english.txt")  # 原始数据 初始文件
    parser.add_argument('-trg_data',default="./data/english.txt")  # 原始数据 目标文件
    ...
    parser.add_argument('-epochs', type=int, default=10)  # 要为训练多少个epochs数据(默认值为2)
    ...
    parser.add_argument('-printevery', type=int, default=1)  # 在打印前运行多少次迭代(默认值为100)
    ...
    parser.add_argument('-checkpoint', type=int, default=3)  # 输入若干分钟。每隔几分钟模型的权重就会被保存到文件夹'weights/'
